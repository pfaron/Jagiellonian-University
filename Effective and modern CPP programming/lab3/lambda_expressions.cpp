...
int main() {
    ...
    int center = 50;
    std::sort(v.begin(), v.end(), [center](int x, int y) {
    ...
    auto positiveEvenFirst = [](int x, int y) {
        // only for assertion
        if (x == y)
            return false;

        // true - no switch, false - switch
        if (x < 0) {
            if (y >= 0)
                return false;
            ...
            // x<0, y<0, x odd, true if y is odd
            return y % 2 != 0;
        } else if (x == 0) {
            return y <= 0;
        }
    ...
    auto l1_norm = [](auto container) {
        auto abs_sum = [](double acc, double snd) {
            return acc + abs(snd);
        };
        return std::accumulate(container.begin(), container.end(), 0.0, abs_sum);
    };                           // [ 5 ]
